...
void show_node2(struct node2 *border, int total_day, int expd_type)
{
    ...
    { 
        ...
        cout<<"Total Expenditure:\t"<<border->total_expd<<" Rs/-"<<endl;
        no1= (border->total_expd - border->total_dipo);
        //cout<<"Total Due/Extra:\t"<<no1<<" Rs/-; that is: "<<round_off_amount(no1)<<" Rs/-"<<endl; 
           if(no1>=0) cout<<"****Due Cash****=  ";
           else cout<<"^^^Extra Cash^^^=  ";
           cout<<abs(no1) <<" Rs/-; that is: "<<abs(round_off_amount(no1))<<" Rs/-"<<endl; 
           
        //cout<<"Total Due/Extra:\t"<<border->total_due<<endl;       
        amt1+=round_off_amount(no1);      amt2+=border->total_due;
...
float round_off_amount(float number)
{
    int a, fl=1; float decimal_part, no;
    if(number<0) fl= -1;
    number= abs(number);
    a=(int)number;
